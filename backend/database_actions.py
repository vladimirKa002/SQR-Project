...
def create_template(name: str, items: list[int], picture: BLOB, db: Session) -> Template:
...
def create_item(name: str, description: str, price: int, picture: BLOB, db: Session) -> Item:
    item = Item(name=name, description=description, price=price, picture=picture)
    db.add(item)
    db.commit()
    return item
...
def rank_tierlist_item(item_id: int, tier_list_id: int, tier: str, db: Session) -> TierListItem:
    ...
    if item:
        ...
        item = TierListItem(item_id=item_id, tier_list_id=tier_list_id, tier=tier)
    db.add(item)
    db.commit()
    return item


def delete_tierlist_item(item_id: int, tier_list_id: int, db: Session) -> None:
    item = get_tierlist_item(item_id, tier_list_id, db)
    if item:
        db.delete(item)
        db.commit()
